...
def setUp(start_date=ov.date, end_date=None, dt=ov.time_width, timestepBunch=48):
    ...
    if end_date == None:
        coe = f.get_coe_by_date(start_date, f.coe_data)
        bau_load = f.get_load_by_date(start_date, f.load_data)
    else:
        coe = f.concatenate_coe_by_date_range(start_date, end_date, f.coe_data)
        bau_load = f.concatenate_load_by_date_range(start_date, end_date, f.load_data)
    ...
    ### NOTE: THE SAME COP IS USED FOR BOTH CHARGING AND DISCHARGING SET-UPS. 
    ### For different CoP values we will need to edit the objective function 
    for i in range(s):
        ...
        cop_from_temp = f.get_cop_from_temp(f.cop_data)
        #print(f.temperature_data)
        bauCOP[i][2] = float(cop_from_temp(f.get_ambient_temperature(coe[i][0], coe[i][1], f.temperature_data)))
        #print(bauCOP[i][2])
        ...
        dcop_from_temp = f.get_dcop_from_temp(f.dcop_data)
        temp = dcop_from_temp(f.get_ambient_temperature(coe[i][0], coe[i][1], f.temperature_data))
...
def  makeSched(coe, bauLoad, bauCOP, dischargingCOP, combinedLambda, timestepBunch=48, dt=ov.time_width, dyn=False):
    ##############################################################################################
    ...
    J = 0
    for K in kIndex:
        T_amb = f.get_ambient_temperature(coe[K-1][0], coe[K-1][1], f.temperature_data) # coe[K-1][0] and coe[K-1][1] are date and timeperiod respectively.
    ...
    pcmSchedule = [["Date", "HH", "Q_dot", "Mode", "SOC", "dt * CoE", "BAU COE"]]
    ...
    for k in kIndex:
        ...
        costElectricity = f.get_coe_by_date_and_time(coe[k-1][0], hh, f.coe_data)
...
def calcProf(kIndex, start_date, end_date, coe, chargingCOP, dischargingCOP, pcmSchedule, dt=ov.time_width, print_summary=False, print_schedule=False):
    chargingEvents = 0 
    dischargingEvents = 0
    totalChargeCost = 0
    ...
    #     ############ CALCULATE PROFITS #############
        ...
        if pcmSchedule[k][2] > 0:
            ...
            dischargingEvents += 1
            # TO-DO: use compressor electrical power and
            # NOTE: c_COP and d_COP need to be the same at this state of the code
...
if __name__ == "__main__":
    ...
    for i in range(len(starts)):
        ...
        profits, charges, discharges, trades = calcProf(kIndex, starts[i], ends[i], coe, bauCOP, dischargingCOP, sched)
